...
class UserDataSource(private val profileRepository: ProfileRepository, private val compositeDisposable: CompositeDisposable): PageKeyedDataSource<Int, UserData>() {
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, UserData>) {
        updateState(State.LOADING)
        compositeDisposable.add(
            profileRepository.userList(params.key.toString())
                .subscribe({ response ->
                    updateState(State.DONE)
